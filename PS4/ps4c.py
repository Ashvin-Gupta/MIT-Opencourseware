...
class SubMessage(object):
    ...
    def build_transpose_dict(self, vowels_permutation):
        ...
        vowel_perm_list = list(vowels_permutation)
        vowel_perm_list_upper = list(vowels_perm_upper)

        i = 0
        j = 0

        for vowel in VOWELS_LOWER:
            new_dict[vowel] = vowel_perm_list[i]
            i += 1
        for vowel in VOWELS_UPPER:
            new_dict[vowel] = vowel_perm_list_upper[j]
            j += 1
    


        list_constants_lower = list(CONSONANTS_LOWER)
        list_constants_upper = list(CONSONANTS_UPPER)

        i = 0
        j = 0

        for c in list_constants_lower:
            new_dict[c] = list_constants_lower[i]
            i += 1
        for c in list_constants_upper:
            new_dict[c] = list_constants_upper[j]
            j += 1
    ...
    def apply_transpose(self, transpose_dict):
        ...
        for c in message:
            encrypted_message += transpose_dict.get(c,c)
...
'''
        ...
        for perm in perms:
            ...
            trasnposed_dict = self.build_transpose_dict(perm)
            transposed_message = self.apply_transpose(trasnposed_dict)
            length_encrypt_msg = len(self.message_text)

            transposed_list = list(transposed_message)

            for w in transposed_list:
                if w in v_words:
                    current_val_words += 1

            print(current_val_words)

            if current_val_words == length_encrypt_msg:
